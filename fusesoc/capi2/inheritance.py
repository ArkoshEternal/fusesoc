...
class Inheritance:
    ...
    def yaml_merge_2_fusesoc_merge(self, capi: str) -> str:
        """
    ...
    def elaborate_inheritance(self, capi: str | dict) -> str | dict:
        if not isinstance(capi, dict):
            return capi

        for key, value in capi.items():
            if isinstance(value, dict):
                capi[key] = self.elaborate_inheritance(capi=copy.deepcopy(value))

        parent = capi.pop(self.MERGE_OPERATOR, {})
